	...
	public void getphonumber() throws InterruptedException, IOException {
		...
		for (j = 0; j <= sizeofdata - 1; j++) {
			...
			try {
				...
				String errortext = driver.findElementByXPath("//div[@class='error-message']/p").getText();
				System.out.println(errortext);
				if(errortext.equalsIgnoreCase("Another user has logged in to this account. Please log in again.")){

					WebDriverWait wait1 = new WebDriverWait(driver, 60); 
					wait1.until(ExpectedConditions.elementToBeClickable(driver.findElementByXPath("//span[text()='Ok']")));

					driver.findElementById("(//span[@data-ref='btnWrap'])[13]/span/span[2]").click();
					Thread.sleep(3000);
					driver.findElementByName("userEmail").sendKeys("thebroker@usa.com");
					driver.findElementByName("userPW").sendKeys("winter");
