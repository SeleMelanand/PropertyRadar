...
//@Listeners(com.propertyradar.phonenumbercollection.ListernerClass.class)

public class PhonenumberReterivalIdSearchNots extends Loginpropertyradar {
	@Test
	public void getphonumber() throws InterruptedException, IOException {
		...
		for (int j = 0; j <= sizeofdata - 1; j++) {
			...
			Thread.sleep(2000);
			try {
				estimatedvalue = driver
						.findElementByXPath("//div[contains(text(),'Est. Value')]/following::div[1]").getText();
			...
			try {
				assessedvalue = driver
						.findElementByXPath("(//div[contains(text(),'Assessed Value')])[1]/following::div[1]").getText();
			...
			try {
				estimatedtotalloanbalance = driver
						.findElementByXPath("//div[contains(text(),'Total Loan Bal')]/following::div[1]").getText();
			...
			try {
				estimatedequity = driver
						.findElementByXPath("//div[contains(text(),'Est. Equity')]/following::div[1]").getText();
			...
			try {
				purchasedate = driver.findElementByXPath("//div[contains(text(),'Purchase Date')]/following::div[1]").getText();
			...
			try {
				purchaseamount = driver.findElementByXPath("//div[contains(text(),'Purchase Amt')]/following::div[1]").getText();
			...
			try {
				downpayment = driver.findElementByXPath("//div[contains(text(),'Down Payment')]/following::div[1]").getText();
			...
			wait2.until(ExpectedConditions.elementToBeClickable(driver.findElementByXPath("(//span[text()='Search'])[3]")));
